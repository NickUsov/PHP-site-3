<?php
    ...
    $item = "create table items(
        ...
        category_id int, foreign key(category_id) references categories(id) on update cascade,
    ...
    $image = "create table images(
        id int not null auto_increment primary key,
        item_id int, foreign key (item_id) references items(id) on update cascade,
        image_path varchar(255) not null
    ) default charset ='utf8'";

    $sale = "create table sales(
        id int not null auto_increment primary key,
        customer_id int, foreign key (customer_id) references customers(id) on update cascade,
        item_id int, foreign key (item_id) references items(id) on update cascade,
        created timestamp,
